...
- **F8 · S3/E3 — oversized modules.** split `backtest_job.py` (orchestration/persistence/request-models), `broker.py` (order-book+fills vs position-mgmt), `strategies.py` → `strategies/` package; pure moves + re-exports, suites green. **◑ PARTIALLY RESOLVED.** `strategies.py` (891) → `strategies/` **package done** (one archetype per module: pipeline/trend/mean_reversion/momentum/ma_crossover/breakout; `__init__` imports every submodule to fire `@register` and re-exports the public surface — equivalence byte-identical, all suites green). The other two are **deferred with rationale**, not forced: `broker.py`'s `SimBroker` is a single cohesive stateful class (shared `pending`/`positions`/`closed`/`cash`) whose only "split" is mixins over that shared state — an anti-pattern that reduces readability; `backtest_job.py` is well-organized as request→resolve→run triples per job type, and its request models are interleaved with resolvers that share extensive module-level state (`MIN_HISTORY`, `bars_for_duration`, `_build_llm`, `fetch_window`, `_StreamingEngine`, `_cache_dir`), so pulling models out couples two files rather than decoupling one. Forcing either split would trade a long-but-cohesive module for cross-module coupling. Revisit only if a genuine seam emerges (e.g. a second engine reusing broker order-matching, or a non-dashboard consumer of the request models).
...
**Remediation status (2026-07-24):** F1–F7, F9, F10 resolved (F5/F9/F10 verified as already-satisfied; the rest by additive, off-by-default fixes with new tests). **F8 partially resolved** — the `strategies/` package split is done; the `broker.py` / `backtest_job.py` splits are deferred with rationale (forcing them adds coupling, not clarity). Deferred enhancement noted under F5 (walk-forward equity-curve UI headline).
